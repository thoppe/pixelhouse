...
def erosion_distance(canvas, kernel_size=5, depth_iterations=3, decay=.90):
...
C = ph.Canvas(w, h)
C += ph.text("RESIST",x=0,y=.5,
             font="TitilliumWeb-Black.ttf",
             vpos="center",
             font_size=1.25)

dist = erosion_distance(
    C, kernel_size=3,decay=0.8, depth_iterations=20).tolist()
...
C += ph.filters.gaussian_blur(.10, .10)
for y in np.linspace(1.25*C.ymin, 1.25*C.ymax, n):
    C += ph.line(C.xmin, y, C.xmax, y, thickness=tc,gradient=lg)

z = ph.motion.easeReturn("easeInQuad", 0, 0.015, len(C))
C += rise(dist, amplitude=z)

X = C.render(len(C)//2)
X += ph.transform.scale(1./scale, 1./scale)
